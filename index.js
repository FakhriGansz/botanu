...
// auto respon
WibuXploit = ['@'+client.user.jid.split('@')[0]]
for ( var W of WibuXploit){
  if(!mek.isBaileys && budy.match(W)){
    lari = fs.readFileSync('./database/media/sticker/lari.webp')
   //return client.sendMessage(from, lari, sticker, {quoted: mek})
   capt = 'Hai @'+sender.split('@')[0]+' WibuXploit disini'
...
`
  ...
  case 'owner':
  case 'author':
    number = '6281319868981@s.whatsapp.net'
    capt = `Nomor : @${number.split('@')[0]}\n`
    capt += 'Instagram : https://www.instagram.com/Fakhri_awail936'
    await client.fakeLink(from, capt, thumb, 'Click in here', 'https://www.instagram.com/Fakhri_awail936', mek)
   /* client.sendContact(from, '6281319868981', 'owner', {
	 key: {
          fromMe: false,
...
*OWNER*
- Instagram : https://www.instagram.com/Fakhri_awail936
- WhatsApp : wa.me/6281319868981

*SCRIPT*
- Git : ${package.homepage}
- License : ${package.license}` 
  client.fakeLink(from, teks, thumb, tanggal, 'https://www.instagram.com/Fakhri_awail936', mek)
   break
...
if(!isCmd && isChatbot === true){
 ...
 // if(!isPremium) return
  ...
  m.reply(result.success.replace('simsimi', 'WibuXploit').replace('Simsimi', 'WibuXploit').replace('simi', 'WibuXploit').replace('Simi', 'WibuXploit').replace('sim', 'WibuXploit'))
}
